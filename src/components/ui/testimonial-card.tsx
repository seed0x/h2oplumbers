...
export const TestimonialCard = ({ quote, author, rating, source }: TestimonialCardProps) => {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-lg border-2 border-brand-cyan/20 flex flex-col h-full transform hover:-translate-y-2 transition-transform duration-300 hover:shadow-2xl group">
      <div className="flex items-center mb-4">
        {[...Array(5)].map((_, i) => (
          <Star
            key={i}
            className={`w-5 h-5 ${i < rating ? 'text-brand-cyan' : 'text-slate-300'}`}
            fill="currentColor"
          />
        ))}
      </div>
      ...
      <div className="mt-auto">
        ...
        <div className="flex items-center justify-end mt-4 pt-4 border-t border-brand-cyan/20">
